...
/*
    page rename script, version 2.0.21
...
SDV($PageRenameFmt,
    ...
    <p class='vspace'>$[Unqualified Links]:</p>
    \$UnqualifiedLinks");
...
Markup_e('linkslist', 'directives','/\\(:linkslist\\s+(?:(publish)=)?(.*?)\\s*:\\)/i',
  "'<:block>'.Keep(FmtLinksList(MakePageName(\$pagename,\$m[2]), array('o' => 'fmt=pgbygroup','list'=>'all','action'=>\$m[1])))");

Markup_e('includeg','>if',
    "/\\(:includeg\\s+($GroupPattern(?:[\\/.])$NamePattern)(.*?):\\)/",
    "PRR().IncludeGText(\$pagename,\$m[1],\$m[2])");
...
function IncludeGText($pagename,$page,$opts) {
  ...
  return preg_replace_callback(
    ...
    function ($m) use ($ogroup,$ngroup) {
      return QualifyUnqualifiedLinks($ngroup,$ogroup,$m[0]); },
    IncludeText($pagename,"include $page$opts"));
}

function HandlePageLinks($pagename) {
  global $HandlePageLinksFmt,$PageStartFmt,$PageLinksFmt,$PageEndFmt,$FmtV;
...
function FmtLinksList($pagename,$opt) {
  ...
  global $PDFCheckboxFmt,$PDFTypesetVFmt,$PDFOptionsFmt,$HTMLVSpace;
  ...
  if ($opt['action']=='publish') {
      ...
      $FPLByGroupStartFmt =
    "$HTMLVSpace<form class='publish' action='\$ScriptUrl' method='get'>
    <input type='hidden' name='n' value='\$FullName' />
    <dl class='fplbygroup'>";
      $FPLByGroupEndFmt = "</dl>$HTMLVSpace
    <input type='hidden' name='action' value='publish' />
    <input type='hidden' name='ptype' value='searcha' />
    <input type='hidden' name='page' value='\$Group/\$Name' />
    <input type='hidden' name='include' value='include' />";
      $FPLByGroupEndFmt .= "$PDFCheckboxFmt$PDFTypesetVFmt$PDFOptionsFmt</form>";
      $FPLByGroupIFmt = 
...
function FPLLinksByGroup($pagename, &$matches, $opt) {
  ...
  foreach($matches as $pc) {
    ...
    if (PageExists($pc['pagename'])) $out[] = ($pc['pagename']==$pagename) ? 
...
function ListPageLinks($pagename,$list) {
  ...
  for ($i=0;$i<count($m[0]);$i++) {
    $p = QualifyUnqualifiedLinks($g,$dummy,preg_replace_callback(
        "/\\{(\\$.*?)\\}/",
        function ($m) use ($pagename) {
          return FmtPageName($m[1],$pagename); },
        $m[0][$i]));
    if (strstr($p,$dummy)) {
